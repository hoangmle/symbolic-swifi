//see: https://www.irit.fr/wiki/lib/exe/fetch.php?media=wtc:benchmarks:tcas_with_prop_annots.txt
#include "error_lib.h"
...
int alt_sep_test()
{
  ...
  _Bool p1a_cond = (Up_Separation >= Positive_RA_Alt_Thresh_2) &
  					(Down_Separation < Positive_RA_Alt_Thresh_2);
  					
  _Bool p1b_cond = (Up_Separation < Positive_RA_Alt_Thresh_2) &
  					(Down_Separation >= Positive_RA_Alt_Thresh_2);
  					
  _Bool p2a_cond = (Up_Separation < Positive_RA_Alt_Thresh_2) &
  					(Down_Separation < Positive_RA_Alt_Thresh_2) &
  					(Up_Separation > Down_Separation);
  					
  _Bool p2b_cond = (Up_Separation < Positive_RA_Alt_Thresh_2) &
  					(Down_Separation < Positive_RA_Alt_Thresh_2) &
  					(Up_Separation < Down_Separation);
  					
  _Bool p3a_cond = (Up_Separation >= Positive_RA_Alt_Thresh_2) &
  					(Down_Separation >= Positive_RA_Alt_Thresh_2) &
  					(Own_Tracked_Alt > Other_Tracked_Alt);
  					
  _Bool p3b_cond = (Up_Separation >= Positive_RA_Alt_Thresh_2) &
  					(Down_Separation >= Positive_RA_Alt_Thresh_2) &
  					(Own_Tracked_Alt < Other_Tracked_Alt);
  					
  _Bool p4a_cond = (Own_Tracked_Alt > Other_Tracked_Alt);
  
  _Bool p4b_cond = (Own_Tracked_Alt < Other_Tracked_Alt);
  
  _Bool p5a_cond = (Up_Separation > Down_Separation);
  
  _Bool p5b_cond = (Up_Separation < Down_Separation);
  ...
  CHECK (!p1a_cond | (alt_sep != 2));
  CHECK (!p2a_cond | (alt_sep != 2));
//  CHECK (!p3a_cond | (alt_sep != 2));
//  CHECK (!p4a_cond | (alt_sep != 2));
  CHECK (!p5a_cond | (alt_sep != 2));
  
  CHECK (!p1b_cond | (alt_sep != 1));
  CHECK (!p2b_cond | (alt_sep != 1));
  CHECK (!p3b_cond | (alt_sep != 1));
//  CHECK (!p4b_cond | (alt_sep != 1));
  CHECK (!p5b_cond | (alt_sep != 1));
...
int main(int argc, char *argv[])
{
  ...
  klee_assume (Own_Tracked_Alt >= 0);
  klee_assume (Other_Tracked_Alt >= 0);
  initialize();
